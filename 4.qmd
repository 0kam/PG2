...
それではRStudioを開いて、サンプルコードをコンソールに打ち込みながらRの使い方を学んでいきましょう！ ここではファイルは作成せずに、直接コンソールにスクリプトを打ち込みます。何が起こっているのか理解するために、一行ずつ進めるようにしましょう。

RStudioの操作方法について、（途中までですが）スクリーンレコードを撮りました。 何をしていいかわからなくなった際の参考になると思います。

{{< video https://youtu.be/hoyYKPs2Pag >}}
...
関数は自作することもできます。よく使う処理を関数としてまとめておくと便利でしょう。

関数の作成はfunction( ){ }で行い、()の中には関数が受け取る値（引数）を、{}には引数をどのように処理するかを書きます。

関数は他の値と同じように、変数として扱うことができます（これ、Rの結構珍しい特徴です）。
...
Rは統計解析、地理情報解析、データ可視化などの様々なジャンルで活用されています。Rでは、ユーザーが自分の開発した解析方法などをまとめて誰でも使える関数群として公開することができ、これを「ライブラリ」と呼びます。Rライブラリが集まっている[CRAN](https://cran.r-project.org/)と呼ばれるウェブサイトには、22,000を超えるライブラリが登録されており、Rが非常に広い用途で使われていることがわかります。

この授業でも、主にデータの処理と地理情報解析用のライブラリをいくつか使います。 ライブラリのインストールは、コンソールに`install.packages("ライブラリ名")`（ライブラリ名は`""`で囲む必要あり）と打って行います。

ライブラリは無数にあるので、関数の名前が衝突してしまうことがあります。例えばAライブラリとBライブラリが別々の動作をする関数を、`filter()`と言う名前で登録してしまった、と言う感じです。

こういった混乱を減らすために、Rはユーザーが指定したライブラリのみを読み込むように設計されています。

したがって、Rスクリプトで特定のライブラリに含まれる関数を使う際には、冒頭に「そのライブラリを読み込む」ためのスクリプトを書く必要があります。

インストール済みのライブラリを呼び出すには、以下のように、`library()`関数を使います。
...
# 前回インストールした tidyverse ライブラリを読み込む。
# install.packages() と異なり、"" は不要
library(tidyverse)
```
...
まず、RStudioの左上にある「プロジェクト新規作成」ボタンを押し、`Existing Directory`から`PG2/4`を選びます。

次に、「新規作成」ボタンを押して`RScript`を選びます。左上のペインに新しいファイルが表示されたはずです。ここにスクリプトを書き込んで行きます。書き込んだら「保存」ボタンを押して名前（`r_fundamental`とか）をつけて保存しましょう。

{{< video https://youtu.be/diCdp3xu1KQ >}}
...
`penguins`のデータを使って、新しいグラフを書いてみましょう。`mutate()`を少なくとも1回使って、新しい列を作るようにしてください。
...
それでは、RStudioに移ります。今回は、前回同様、`PG2/4`での作業です。したがって、前回作成したRプロジェクトをそのまま使います。

`PG2/4`を覗いてみると、`4.Rproj`と言うファイルができていませんか？これをダブルクリックすると、このプロジェクトが開かれた状態でRStudioを立ち上げることができます。

まずRScriptを作成します。RStudioの左上にある新規作成ボタン（白紙に緑のプラスマーク）を押し、`R Script`を選択します。「保存」ボタンを押すと、保存先のファイル名を聞かれるので、`1_read_dem`として保存します。

それでは、以下をコピペして上から順に実行してみましょう。右下のペインにDEMが表示されるはずです。
...
setwd("~/Documents/PG2/4/") #| hide_line
library(terra)
library(tidyverse)
library(tidyterra)
...
### 市区ごとの標高をグラフにする {#sec-elevation-by-cities}
